...
<?php
	...
	if(sizeof($_POST)>0){
		...
		if ($_POST["nome_pj"]!=""&&$_POST["nome"]!=""&&$_POST["email"]!=""&&$_POST["nascimento"]!=""&&$_POST["senha"]!=""&&$_POST["confSenha"]!=""){
			...
			if($senha != $confSenha){
				...
				if (!$res){
					$msg = '<h3>'.mysqli_error().'</h3>';
				}else{
					if (mysqli_num_rows($res)>0){
						$msg = '<h3>Este e-mail já está cadastrado</h3>';
					}else{
						$ins = "insert into usuario (nome_pj, nome, email, senha, data_nascimento)
								values ('$nome_pj','$nome','$email',md5('$senha'), str_to_date('$nascimento','%d/%m/%Y'))";
...
<html>
	...
	<body>
		<h1>Cadastro</h1>
		<form method="post">
			<label for="nome_pj">Nome do PJ: </label>
			<input type="text" id="nome_pj" name="nome_pj" value="<?=$nome_pj?>" placeholder="nome_pj"><br /><br />
